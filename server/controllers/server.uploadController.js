...
exports.crashes = async function(req, res){
    ...
    if(req.id === -1){
        ...
        if(!adminStatus){
            return res.status(401).send("No admin access");
        }
        let file = req.files.file;
        console.log(file);
        file.mv(process.cwd()+`/server/adminData/`+file.name+Date.now(), function(err) {//make this a database later
            if (err) {
...
exports.quizContent = async function(req, res){
    ...
    if(req.id === -1){
       ...
       await User.findById(req.id,function(err,user){
        ...
        let file = req.files.file;
        console.log(file);
        file.mv(process.cwd()+`/client/src/views/Quiz/UploadData`+file.name+Date.now(), function(err) {//make this a database later
            if (err) {
                return res.status(500).send(err);
            }
            res.status(200).send('200 got file');
...
exports.userQuizResult =async function(req, res){
    ...
    if(req.id === -1){
        ...
        if(adminStatus){
            return res.status(401).send("Admins can't have quiz results");
        }
